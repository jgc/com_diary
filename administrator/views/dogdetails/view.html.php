 ...
 */
class DiaryViewDogdetails extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		DiaryHelper::addSubmenu('dogdetails');
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_DIARY_TITLE_DOGDETAILS'), 'dogdetails.png');

        //Check if the form exists before showing the add/edit buttons
        $formPath = JPATH_COMPONENT_ADMINISTRATOR.'/views/dogdetail';
        if (file_exists($formPath)) {

            if ($canDo->get('core.create')) {
			    JToolBarHelper::addNew('dogdetail.add','JTOOLBAR_NEW');
		    }

		    if ($canDo->get('core.edit') && isset($this->items[0])) {
			    JToolBarHelper::editList('dogdetail.edit','JTOOLBAR_EDIT');
		    }
		...
		if ($canDo->get('core.edit.state')) {
            ...
            if (isset($this->items[0]->state)) {
			    ...
			    JToolBarHelper::custom('dogdetails.publish', 'publish.png', 'publish_f2.png','JTOOLBAR_PUBLISH', true);
			    JToolBarHelper::custom('dogdetails.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
            } else if (isset($this->items[0])) {
                //If this component does not use state then show a direct delete button as we can not trash
                JToolBarHelper::deleteList('', 'dogdetails.delete','JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->state)) {
			    JToolBarHelper::divider();
			    JToolBarHelper::archiveList('dogdetails.archive','JTOOLBAR_ARCHIVE');
            }
            if (isset($this->items[0]->checked_out)) {
            	JToolBarHelper::custom('dogdetails.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
            }
		}
        ...
        if (isset($this->items[0]->state)) {
		    if ($state->get('filter.state') == -2 && $canDo->get('core.delete')) {
			    JToolBarHelper::deleteList('', 'dogdetails.delete','JTOOLBAR_EMPTY_TRASH');
			    JToolBarHelper::divider();
		    } else if ($canDo->get('core.edit.state')) {
			    JToolBarHelper::trash('dogdetails.trash','JTOOLBAR_TRASH');
			    JToolBarHelper::divider();
		    }
        }
		...
		JHtmlSidebar::setAction('index.php?option=com_diary&view=dogdetails');
